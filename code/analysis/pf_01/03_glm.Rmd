...
The `mnth_since_vac` variable is updated so that we take 07-2008 as the start of vax (i.e. drop the first year). The `incl` flags the observations we should include the models.
...
```{r}
get_dat <- function(x){
  return(list(desc = x[[1]], model_res = broom::tidy(x[[2]])))
}
